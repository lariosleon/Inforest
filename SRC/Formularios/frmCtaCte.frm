...
Private Sub grdGrilla_DblClick()
    If RsCabecera.RecordCount > 0 Then
       sCodigo = grdGrilla.Columns(0).Text
       frmCtaCteDetalle.Show vbModal
    Else
...
Private Sub cmdOpcion_Click(Index As Integer)
    Select Case Index
           ...
           Case Is = 5  'Emite
                ...
                Reporte.Text3.SetText "Estado"
                Reporte.Text1.SetText "Cliente"
                Reporte.Text4.SetText "Fecha"
                Reporte.Text19.SetText "Creacion"
...
Private Sub Form_Load()
    ...
    ' Ingrese el Titulo
    Me.Caption = " Correlativo de Cuentas Corrientes "
    ...
    'lgcentralpedido
    Cn.Execute "update TDELIVERY set nConsumo=0"
    Cn.Execute "update TDELIVERY set nConsumo = T1.nVenta FROM (SELECT SUM(dbo.DPEDIDO.nVenta) AS nVenta, dbo.MPEDIDO.tClienteCtaCte FROM dbo.MPEDIDO INNER JOIN dbo.DPEDIDO ON dbo.MPEDIDO.tCodigoPedido = dbo.DPEDIDO.tCodigoPedido WHERE     (dbo.MPEDIDO.tEstadoPedido = '04') GROUP BY dbo.MPEDIDO.tClienteCtaCte) T1 Where TDELIVERY.TCODIGODELIVERY = T1.tClienteCtaCte"
    
    ' Ingrese la Vista
    Isql = "select Descripcion, nConsumo, nLinea, nSaldo, Codigo, fRegistro, lActivo,  tNomSoc, tDireccion, tTelefono,   tEmail, tUsuario, EstadoFrecuente, " & _
           "str(nLinea,10,2) as xLinea, str(nConsumo,10,2) as xConsumo, str(nSaldo,10,2) as xSaldo " & _
           "from vCompania"

    Set RsCabecera = Lib.OpenRecordset(Isql, Cn)
    'Configuración de la Grilla
    Call ConfGrilla(8, grdGrilla, "Codigo", 2, "Codigo", 1000, 2, 0, "", _
                                  "Cliente", 2, "Descripcion", 4000, 0, 0, "", _
                                  "Direccion", 2, "tDireccion", 3800, 0, 0, "", _
                                  "Telefono", 2, "tTelefono", 1600, 0, 0, "", _
                                  "Linea", 2, "xLinea", 1200, 1, 0, "###,##0.00", _
                                  "Consumo", 2, "xConsumo", 1200, 1, 0, "###,##0.00", _
                                  "Saldo", 2, "xSaldo", 1200, 1, 0, "###,##0.00", _
                                  "Estado", 2, "EstadoFrecuente", 1500, 0, 0, "")
