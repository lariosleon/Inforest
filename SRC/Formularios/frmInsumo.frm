...
Dim Reporte As New dsrRepInsumosPtoVenta
Dim ReporteAdmin As New dsrRepInsumos
...
'Private Sub cmdActStock_Click()
'On Error GoTo fin
'        Dim rsSAP As Recordset
'        Dim r As Integer
'        If lSAP Then
'            Isql = "exec sp_VinculacionSAP '" & sServidorSAp & "','" & sBdSAP & "','" & sCodSap & "','','',5"
'            Set rsSAP = Lib.OpenRecordset(Isql, Cn)
'            If rsSAP.RecordCount > 0 Then
'               rsSAP.MoveFirst
'               Do While Not rsSAP.EOF
'               ' s = "exec sp_VinculacionSAP '" & sServidorSAp & "','" & sBdSAP & "','" & sCodSap & "','" & rsSAP!inf_codigoexterno & "','',7"
'                   Cn.Execute "exec sp_VinculacionSAP '" & sServidorSAp & "','" & sBdSAP & "','" & rsSAP!tcodigoinsumo & "','" & rsSAP!inf_stock & "','',6"
'                   Cn.Execute "exec sp_VinculacionSAP '" & sServidorSAp & "','" & sBdSAP & "','" & sCodSap & "','" & rsSAP!inf_codigoexterno & "','',7"
'                rsSAP.MoveNext
'                Loop
'            MsgBox rsSAP.RecordCount & " Datos Importados"
'            Else
'
'                MsgBox "No Hay Datos A Importar"
'            End If
'         'Cn.Execute "exec sp_VinculacionSAP '" & sServidorSAp & "','" & sBdSAP & "','" & sPasa & "','" & sCodigo & "','',5"
'        End If
'    Exit Sub
'fin:
'MsgBox "error en el procesamiento"
'End Sub
...
Private Sub cmdOpcion_Click(Index As Integer)
    Select Case Index
        ...
        Case Is = 5 'Emite
                ...
                RsReporte.Sort = grdGrilla.Columns(2).DataField & " ASC"
                ...
                Else
                    ReporteAdmin.Database.SetDataSource RsReporte
                    ReporteAdmin.SelectPrinter Printer.DriverName, Printer.DeviceName, Printer.Port
                    ReporteAdmin.DiscardSavedData
                    ReporteAdmin.ReportTitle = Me.Caption
                    frmEmite.CRViewer.ReportSource = ReporteAdmin
                End If
...
Private Sub Form_Load()
    ...
    Select Case sModulo
        Case "INFOREST"
            Call ConfGrilla(6, grdGrilla, "Codigo", 2, "TCODIGO", 1800, 2, 0, "", _
                                "Usuario", 2, "tusuarioReg", 1800, 0, 0, "", _
                                "Descripcion", 2, "DESCRIPCION", 3500, 0, 0, "", _
                                  "Cant. Disp", 2, "nStock", 1000, 0, 0, "", _
                                  "Fecha", 2, "fRegistro", 2000, 0, 0, "dd/MM/yyyy hh:mm:ss", _
                                    "Activo", 2, "lactivo", 750, 2, 4, "")
        Case "ADMINISTRACION"
            Call ConfGrilla(5, grdGrilla, "Codigo", 2, "TCODIGO", 1800, 2, 0, "", _
                                  "Usuario", 2, "tusuarioReg", 2000, 0, 0, "", _
                                  "Descripcion", 2, "DESCRIPCION", 4000, 0, 0, "", _
                                  "Fecha", 2, "fRegistro", 2300, 0, 0, "dd/MM/yyyy hh:mm:ss", _
                                    "Activo", 2, "lactivo", 750, 2, 4, "")
        
        Case Else
    End Select
    
    RsCabecera.Sort = grdGrilla.Columns(1).DataField & " ASC"
    Set grdGrilla.DataSource = RsCabecera
    
'     If lSAP Then
'       cmdActStock.Visible = True
'       Else
'       cmdActStock.Visible = False
'     End If
