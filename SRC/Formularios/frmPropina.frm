...
Begin VB.Form frmPropina 
   BackColor       =   &H00C0C0C0&
   ClientHeight    =   7395
   ClientLeft      =   -3525
   ClientTop       =   -1125
   ClientWidth     =   11880
   Icon            =   "frmPropina.frx":0000
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MDIChild        =   -1  'True
   ScaleHeight     =   7370.101
   ScaleMode       =   0  'User
   ScaleWidth      =   11880
...
Private Sub cmdOpcion_Click(Index As Integer)
    Select Case Index
           Case Is = 0  'Nuevo
               ...
               xDocumento = RsCabecera!tDocumento
               xCorrelativo = RsCabecera!tCorrelativo
               
               sTipo = "Prepintado"
               sCodigo = RsCabecera!nPropina
               frmNumPad.Show vbModal
               If wEnter Then
                  nPos = RsCabecera.Bookmark
                  Cn.Execute "update DPAGODOCUMENTO set nPropina = " & Val(sDescrip) & " where tDocumento ='" & xDocumento & "' and tCorrelativo = '" & xCorrelativo & "'"
                  RsCabecera.Requery
           ...
           Case Is = 2  'Buscar
                If Len(cboCriterio) > 0 And Len(Trim(txtCriterio)) > 0 Then
                   Select Case VarType(grdGrilla.Columns(cboCriterio.ItemData(cboCriterio.ListIndex)).Value)
                          ...
                          Case 7
                          If VarType(txtCriterio.Text) <> 7 And VarType(txtCriterio.Text) <> 8 Then
                             Screen.MousePointer = vbDefault
                             MsgBox "Formato permitido: DD/MM o DD/MM/AAAA", vbExclamation, sMensaje
                             Exit Sub
                          End If
                ...
                If Len(cboCriterio) > 0 And Len(Trim(txtCriterio.Text)) > 0 Then
                   Select Case VarType(grdGrilla.Columns(cboCriterio.ItemData(cboCriterio.ListIndex)).Value)
                          ...
                          If VarType(txtCriterio.Text) <> 7 And VarType(txtCriterio.Text) <> 8 Then
                             ...
                             Exit Sub
                          End If
                          CriterioF = Trim(grdGrilla.Columns(cboCriterio.ItemData(cboCriterio.ListIndex)).DataField) & " >= #" & txtCriterio.Text & "# and " & Trim(grdGrilla.Columns(cboCriterio.ItemData(cboCriterio.ListIndex)).DataField) & " <= #" & txtCriterio.Text & " 23:59#"
                          Case Else
...
Private Sub Form_Load()
    Screen.MousePointer = vbHourglass
    ' Ingrese el Titulo
    Me.Caption = " Mantenimiento de Propinas "
    ...
    ' Ingrese la Vista
    Isql = "select * from vliquidacion where tCaja='" & sCaja & "' and tTurno ='" & sTurno & "' and tTipoPago ='02'"
    Set RsCabecera = Lib.OpenRecordset(Isql, Cn)
    
    'Configuración de la Grilla
    Call ConfGrilla(8, grdGrilla, "Documento", 2, "tDocumento", 1550, 0, 0, "", _
                                  "Fecha Pago", 2, "fPago", 1500, 0, 0, "dd/mm/yyyy HH:nn", _
                                  "Venta", 2, "nVenta", 1250, 1, 0, "###,###,##0.00", _
                                  "Tarjeta", 2, "Tarjeta", 1750, 0, 0, "", _
                                  "Fecha Venc", 2, "Tarjeta", 1500, 0, 0, "", _
                                  "Número", 2, "tNumero", 1100, 0, 0, "", _
                                  "Abono", 2, "nMonto", 1200, 1, 0, "###,###,##0.00", _
                                  "Propina", 2, "nPropina", 1200, 1, 0, "###,###,##0.00")
                                    
    'grdGrilla.Columns(1).DataType = 7  ' Fecha
    'grdGrilla.Columns(2).DataType = 5  ' Numerico
    'grdGrilla.Columns(5).DataType = 5  ' Numerico
    'grdGrilla.Columns(6).DataType = 5  ' Numerico
