...
Begin VB.Form frmOferta 
   BackColor       =   &H00C0C0C0&
   ClientHeight    =   8145
   ClientLeft      =   2535
   ClientTop       =   1725
   ClientWidth     =   13770
   Icon            =   "frmOferta.frx":0000
   LinkTopic       =   "Form1"
   MDIChild        =   -1  'True
   ScaleHeight     =   8145
   ScaleWidth      =   13770
...
Private Sub cmdOpcion_Click(Index As Integer)
    Select Case Index
           ...
           Case Is = 5  'Modifica
                ...
                Screen.MousePointer = vbHourglass
                Isql = "SELECT dbo.TOFERTA.tOferta, dbo.TOFERTA.tNombre, dbo.vProducto.Grupo, dbo.vProducto.SubGrupo, dbo.vProducto.Descripcion AS Producto, dbo.TOFERTA.tFrecuencia, dbo.vFrecuencia.Descripcion AS Frecuencia, dbo.TOFERTA.fFecha, dbo.TOFERTA.tHoraInicial, dbo.TOFERTA.tHoraFinal, dbo.TOFERTA.lAcumulable, dbo.TOFERTA.nRatio, dbo.TOFERTA.nMonto, dbo.TOFERTA.lPermanente, dbo.TOFERTA.fFechaInicial, dbo.TOFERTA.fFechaFinal , dbo.TOFERTA.lActivo " & _
                       "FROM dbo.TOFERTA INNER JOIN dbo.vProducto ON dbo.TOFERTA.tCodigoProducto = dbo.vProducto.Codigo LEFT OUTER JOIN dbo.vFrecuencia ON dbo.TOFERTA.tFrecuencia = dbo.vFrecuencia.Codigo " & _
                       "Order by tNombre, Grupo, SubGrupo, Producto"
                Set RsOferta = Lib.OpenRecordset(Isql, Cn)
                Reporte.DiscardSavedData
                Reporte.ReportTitle = Me.Caption
                Reporte.Database.SetDataSource RsOferta
                ...
                Reporte.Text25.SetText localConectado
...
Private Sub Form_Load()
    ...
    ' Ingrese la Vista
    Isql = "select *, str(nRatio,10,2) as xRatio, str(nMonto,10,2) as xMonto, str(nPrecio,10,2) as xPrecio from vOferta"
    
    Set RsCabecera = Lib.OpenRecordset(Isql, Cn)
    
    'Configuración de la Grilla
    Call ConfGrilla(9, grdGrilla, "Codigo", 2, "tOferta", 1000, 2, 0, "", _
                                  "Oferta", 2, "tNombre", 3800, 0, 0, "", _
                                  "Fec.Especial", 2, "fFecha", 1400, 0, 0, "dd MMMM", _
                                  "Hor.Ini", 2, "tHoraInicial", 1000, 2, 0, "", _
                                  "Hor.Fin", 2, "tHoraFinal", 1000, 2, 0, "", _
                                  "Perman.", 2, "lPermanente", 1000, 2, 4, "", _
                                  "Ratio", 2, "xRatio", 1000, 1, 0, "##0.00", _
                                  "Monto", 2, "xMonto", 1000, 1, 0, "###,##0.00", _
                                  "Precio", 2, "xPrecio", 1000, 1, 0, "###,##0.00")
