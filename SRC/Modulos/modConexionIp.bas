Attribute VB_Name = "modConexionIp"
Option Explicit
...
Public Function obtieneDireccionIp(direccion As String) As String
    Dim cadena() As String
    Dim ip As String
   ' cadena = InStr(1, direccion, "/")
    cadena = Split(direccion, "\")
    ip = cadena(0)
    obtieneDireccionIp = ip
End Function

Public Function conectaPCServidor(direccionIp As String) As Boolean
On Error GoTo sale:
 Dim Reply As ICMP_ECHO_REPLY
   Dim lngSuccess As Long
   Dim strIPAddress As String
   
   strIPAddress = obtieneDireccionIp(direccionIp)
   
   'Get the sockets ready.
   If SocketsInitialize() Then
      
    'Address to ping
  '  strIPAddress = Trim(Me.txtip.Text)
    ...
    lngSuccess = ping(strIPAddress, Reply)
     ...
     conectaPCServidor = True
    Else
     conectaPCServidor = False
