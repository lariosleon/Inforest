...
Begin VB.Form frmBusca 
   ...
   Begin VB.ComboBox cboCriterio 
      Height          =   315
      ItemData        =   "frmBusca.frx":25D2
      Left            =   30
      List            =   "frmBusca.frx":25D4
...
Private Sub txtCriterio_Click()
    If TipoOperacion = "UBIGEO" Then
        frmKeyBoard.txtResultado = txtCriterio.Text
        frmKeyBoard.Show vbModal
        Me.txtCriterio.Text = IIf(wEnter, sDescrip, txtCriterio.Text)
        Me.Filtrar
...
Public Sub Aceptar()
   If Rs.RecordCount > 0 Then
      wEnter = True
      scodigo = IIf(IsNull(Rs!codigo), "", Rs!codigo)
      sDescrip = IIf(IsNull(Rs!Descripcion), "", Rs!Descripcion)
      TipoOperacion = ""
      Unload Me
   End If
